...
- Prefer direct string literals, including long format strings with implicit captures. Use `concat!` for meaningful text groups or a demonstrated local rustfmt workaround; [string literals](docs/development-checks.md#string-literals) explains value preservation and the backslash continuations that `eden-fmt` refuses.

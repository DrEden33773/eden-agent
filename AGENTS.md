...
- Carry long strings with `concat!`, a byte string as `concat!(...).as_bytes()`, or a raw string laid out by meaning. `eden-fmt` refuses a backslash continuation and never rewrites one; [string literals](docs/development-checks.md#string-literals) states what it accepts and why a format string needs its captures written out.

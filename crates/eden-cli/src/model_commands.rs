...
async fn login(
    ...
    let mut reader = if interaction.manual_input {
        eprintln!(
            "Paste the authorization code or redirect URL, then press Enter (or finish in the browser)."
        );
        Some(
    ...
    let result=async {
        let value=match cli.family.as_ref().ok_or("missing operation")? {
            ...
            Family::Auth{action}=>match action {
                AuthAction::Login{provider,method}=>login(&session,provider,method.clone()).await?,
                AuthAction::Refresh{provider}=>cancellable(&session,session.authenticate(AuthRequest::Refresh{provider:provider.clone()})?).await?,
                AuthAction::ReadInput=>unreachable!("private input helper returned before session creation"),

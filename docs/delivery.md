...
`eden history export HISTORY BACKUP.jsonl` remains a complete public-history backup. It does not load the original conversation plugins. The separate reading export omits private installation state and is not a restore format.

```sh
eden export HISTORY PREVIEW.html
eden export HISTORY PREVIEW.jsonl --jsonl --selection '{"head":42,"runs":[3,4],"thinking":true,"attachments":true}'
eden share PREVIEW.html --sha256 DIGEST_FROM_EXPORT --confirm
```
...
HTML is self-contained and has no executable scripts or external resources. Conversation text is escaped, images use selected inline data, other attachments retain their portable base64 representation, and unrecognized display content falls back to text. Reading JSONL starts with `eden-reading-v1` and `restorable:false`; it is deliberately distinct from backup transactions.
...
The official publisher creates a **secret gist**. Anyone with its URL can access it; secret is not private access control. Download the HTML and open it locally. No Pi viewer or Eden-hosted viewer is required. Publication never retries automatically: a lost response, late cancellation or ambiguous server failure reports `PublicationUnknown`, because the gist might already exist. Check the account before explicitly retrying. Cancelling does not promise to undo a remote creation.

...
Each native job runs verification-infrastructure regressions, dependency-license preflight, the platform-sensitive test suites with `--no-fail-fast`, the independent author tests that workspace excludes, Clippy including independent authors, and real Git hook scenarios. The platform-sensitive set is `eden-process`, `eden-coding-tools`, `eden-search`, `eden-local-history` and `eden-workspace`, because those targets own the branches that differ per operating system: process ownership and shell descendants, link creation, path scope and history locking. The Linux runner then runs the whole workspace with those five packages excluded, so it still executes every test the previous single workspace run executed (52 + 69 = 121). The excluded crates are pure logic or temp-directory fixtures. `--exclude` is deliberate: a newly added crate is then covered by the Linux run instead of by no run at all, and `eden-workspace` stays whole on every runner because a test-name filter that selects zero tests would degrade silently. Clippy still builds every target on every runner, and the acceptance suites below still load and exercise every plugin natively on all three, so this layering narrows which tests execute where, not which code compiles or loads where.

The Windows runner previously compiled zero `eden-process` tests: `crates/eden-process/src/tests.rs` is Unix-only, and the job-object paths it parallels were covered only through an installed acceptance suite. `crates/eden-process/src/windows_tests.rs` now asserts them directly, including that completing a command stops its descendants without rewriting the leader's own exit status, that a requested stop ends the leader too, that descendants started during cleanup are still stopped, and that abandoning the tree stops its job. Those tests report a descendant's Windows process id from the shell, because Git Bash's `$!` names an MSYS pid: the native runner measured `$!` = 787 for the process Windows knows as 5284.

The layering does stop executing the other crates' tests off Linux: Windows runs 49 of the 114 tests it ran before it, and macOS 52 of 121. Compilation and native loading are unchanged, and every plugin is still loaded and exercised natively on all three runners by the acceptance suites below. All five `eden-kernel` tests leave the Windows runner, including the native stop-once test that uses a mock ABI with no platform branch — the coverage loss the layering decision accepted explicitly.

...
`models refresh` fetches Radius `/v1/config` and Copilot `/models` when their credentials are configured. Radius returns the inference base URL and pi-messages models; it has no fabricated universal model list. Dynamic caches are separated by gateway and credential account scope; the previous valid account catalog survives a failed refresh and can be read offline. Refresh never changes the active run's frozen target. Configure a different Radius gateway through `credentials.oauth.radius.gateway`; this also selects its catalog endpoint. Set `catalog.providers.radius.base_url` only when intentionally overriding both catalog and inference routing. Explicit model/provider configuration remains highest priority.

## Local model management

The [llama.cpp router guide](router.md) covers discovery, search/download, load/unload, cancellation, reconnect and SDK operations. Only currently routable models are selectable; credentials, catalog, manager and inference remain independently replaceable.

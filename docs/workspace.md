...
Discovery injects only the name, description and source path into model context. Bodies are retained in the immutable resource snapshot and are sent only when the model loads them or the user submits `/skill:check arguments`. Model context advertises `skill` when enabled; otherwise, when `read` is enabled, it advertises `read` with `eden-resource://skill/{name}`. This resource URI returns the complete frozen Skill body without reopening its disk path, and still enforces `disable-model-invocation`. Ordinary file paths remain disk reads. When neither loading tool is enabled, context omits the Skill catalog. Relative references resolve from the skill directory. `disable-model-invocation: true` hides the skill from model invocation while retaining explicit user invocation. Duplicate names keep the first source and produce a diagnostic.

Templates come from global `prompts/`, trusted `.eden/prompts/`, and explicit paths. `/filename arguments` expands that Markdown template. A leading UTF-8 BOM is accepted in Skill and template Markdown. Arguments support single/double quote grouping and limited escaping: outside quotes, a backslash escapes whitespace or a quote; inside double quotes, it escapes a double quote, and a doubled backslash immediately before the closing quote produces one trailing separator; inside single quotes, backslashes are literal. Other backslashes remain literal, including Windows drive paths and UNC prefixes. Empty quoted arguments are retained. Substitutions support `$1`, `$2`, `$@`, `$ARGUMENTS`, `${3:-default}`, `${@:2:1}` and `${ARGUMENTS:-default}`. Expansion never executes shell syntax.

`--no-context` also disables automatic SYSTEM/APPEND_SYSTEM loading. `--no-context`, `--no-skills` and `--no-templates` disable their automatic discovery. Explicit skill/template paths remain explicit sources. `eden resources list` reports the active resource metadata; its `diagnostics` entries are `{"level": …, "message": …}` records whose level is `info`, `warning` or `error`. The shared API's `reload_resources()` is an idle management operation: it prepares a complete replacement before publishing it, and a failed reload retains the prior snapshot. Each coding run freezes one resource snapshot and tool catalog. Reopening reads current resources at the recorded cwd while retaining historical content that was actually sent to the model.
...
| `include` | grep-only positive relative-path globs; applied before match counts, skipped-file diagnostics and fallback; empty means unrestricted |
| `context` | grep-only surrounding lines, 0–100 each side; overlapping ranges are deduplicated before pagination |
...
| `max_file_bytes` | Directory grep defaults to 5 MiB, maximum 10 MiB; an explicitly selected larger file uses line-oriented exact streaming; fuzzy search retains the limit |
...
The same structured result is available in `ToolResult.details`. Group `matches` contain hits and group `context` contains surrounding lines, rendered as `line- text`; a line appears only once across overlapping ranges and pages. `total_matches` and `matched_so_far` exclude context, `returned` counts all displayed rows, and `returned_matches` counts hits on that page. With context, each file's selected ranges use line order; files retain the order of their first ranked hit.

Explicit large-file exact search reads incrementally inside the owned worker process, with at most 10 MiB retained for a single line and a 512-byte line preview. Larger lines are skipped with `line_size_limit`, and scanning continues to later lines. Patterns that can consume newlines are reported as `multiline_pattern_unsupported_for_large_file`; file-boundary anchors such as `\A`, `\z`, or anchors inside `(?-m:...)` are reported as `file_anchor_unsupported_for_large_file`. Ordinary indexed files retain their existing behavior. These conditions set `complete: false` rather than claiming complete zero matches. Cancellation waits for worker exit, including during a large line or context scan. Full matching rows and diagnostic pages are cached until cursor eviction; the line buffer limit does not impose a total match-count limit.
...
Package preparation is an explicit command contributed by `distribution`. It requires a bundle with root `package.json` containing a native `manifest`, a text-only `resources` manifest, or both, plus optional `dependencies` and `build`. See [resource package distribution](distribution.md) for text-only and mixed bundles. An archive may have one enclosing directory. `manifest` is the native `PackageManifest`; `library` is relative to the bundle root. Dependencies are source objects with the same forms below. Package links and special files are rejected.
...
Installed versions coexist under global `distribution/packages/<name>/<version>/<target>`. Receipts lock the source, manifest, dependencies and an unambiguous SHA-256 tree digest. The same version cannot be overwritten with different bytes. Package ownership follows this directory layout, including directory aliases; unrelated ancestor receipt files remain ordinary data. Resolve and session load verify managed installed packages against their receipt before resolving library redirects. Missing or corrupt receipts and redirected package contents remain integrity failures. Saved composition references and actual saved session references are reported before removal; `--force` explicitly removes a referenced version while preserving history files. During a binding update, a pending reference protects both old and proposed libraries; a failed update can conservatively retain both until a successful reopen reconciles the reference.
...
Automatic Skill discovery skips hidden entries, `node_modules`, and paths excluded by `.gitignore`, `.ignore` or `.fdignore`. Ordinary Markdown entries are accepted only at a discovery root; nested Skills require `SKILL.md`, whose directory ends further Skill discovery. Templates use immediate Markdown files. Explicit resource paths are required and report missing or malformed content.

`skill_excludes` and `template_excludes` accept glob arrays. They match resource names, full paths, or paths relative to a discovery root; for example `"skill_excludes": ["retired", "legacy/**"]` removes selected resources while continuing to discover future additions. These filters also apply to installed package resources.

On first startup, malformed automatically discovered optional Skills/templates produce path-specific warnings and other resources remain available. Explicit resource paths, selected package resources, settings and system instructions remain required. Reload is strict: any malformed selected entry rejects the update and leaves the entire previous snapshot active. Fix the diagnostic and reload again to adopt a new complete snapshot.

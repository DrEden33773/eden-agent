...
pub(super) fn tools() -> Vec<ToolDefinition> {
    [
        ("powershell", "Run native PowerShell without profiles at the explicit cwd. Waits for \
                process-tree cleanup on completion or cancellation.", json!({"type":"object","properties":{"command":{"type":"string"}},"required":["command"],"additionalProperties":false})),
        ("ls",
...
"additionalProperties":false
        ...
        ("skill", "Load an available skill on demand. Relative references resolve from its \
                skill directory.", json!({"type":"object","properties":{"name":{"type":"string"},"arguments":{"type":"string"}},"required":["name"],"additionalProperties":false})),
        (
            "read",

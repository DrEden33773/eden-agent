...
fn catalog() -> Vec<CommandDefinition> {
    [
        (
            "package.install",
            "Prepare a local directory/tar/tar.gz, pinned Git revision or HTTPS archive. \
             Installation does not enable the package.",
            json!({
                "source": { "type": "object" },
        ...
        (
            "package.remove",
            "Remove one installed version. Referenced versions require explicit force; history is \
             retained.",
            json!({
                "name": { "type": "string" },
                "version": { "type": "string" },

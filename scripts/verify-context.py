...
def complete(output, total=15):
    return {"type": "response.completed", "response": {"status": "completed", "output": output,
            "usage": {"input_tokens": total - 5, "output_tokens": 5, "total_tokens": total}}}


def effect():
    return [{"type": "function_call", "call_id": "effect", "name": "bash",
...
class Server:
    ...
    def __init__(self, respond, gate=None):
        self.requests = []
        self.errors = []
        self.markers = []
        ...
        class Handler(http.server.BaseHTTPRequestHandler):
            ...
            def do_POST(self):
                try:
                    ...
                    if status == 200:
                        if gate == index:
                            delta = {"type": "response.output_text.delta", "delta": f"gate-{index}"}
                            self.wfile.write(("data: " + json.dumps(delta) + "\n\n").encode())
                            self.wfile.flush()
                            assert owner.release.wait(90), "cancelled client did not finish gate scenario"
...
def configure(destination, composition, server, name, window=1048576, allowance=393216, keep_recent=None):
    ...
    for package in selected["packages"]:
        ...
        if package["descriptor"]["package"] == "model-access":
            package["config"] = {"endpoint": f"http://{server.address}/responses", "model": "controlled-model",
                                 "api_key_env": "EDEN_CONTEXT_KEY", "context_window": window,
...
def main():
    ...
    run(["cargo", "build", "-p", "eden-agent", "--example", "context_probe", "--locked"], ROOT)
    ...
    with tempfile.TemporaryDirectory(prefix="eden-context-") as temp:
        ...
        def task(config, history, prompt="Continue task", extras=()):
            return run([host, "--composition", config, "--cwd", project, "--session", history,
                        "--json", *extras, prompt], caller)

        def command(config, history, name, *options):
            return run([host, "session", name, history, "--composition", config, *options], caller)
        ...
        try:
            ...
            assert len(short.requests) == 1, "short manual compaction must not discard recent context"
            ...
            results["default_recent_retention"] = {"keep_recent_tokens": 20000, "short_compaction_requests": 0,
        ...
        image.write_bytes(base64.b64decode("iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mP8/x8AAwMCAO+jR5kAAAAASUVORK5CYII="))
        summary_count = [0]
        ...
        def repeated(body, _):
            if summary(body):
                ...
                return 200, complete(answer(f"summary-marker-{summary_count[0]}: preserve requested goal and image reference"))
        ...
        try:
            ...
            task(config, history, "Remember attached image and ORIGINAL-GOAL", ["--image", image])
            original = records(history)
            image_record = next(r["sequence"] for r in original if r["kind"] == "message" and any(b.get("type") == "image" for b in r["payload"].get("content", [])))
            ...
            assert "ordinary response" in json.dumps(first_projection), "recent response must survive manual compaction"
            assert not any(block.get("type") == "input_image" for item in first_projection for block in item.get("content", []))
            command(config, history, "compact")
            ...
            assert any(block.get("type") == "input_image" for item in server.requests[-1]["input"] for block in item.get("content", []))
            assert records(history)[:len(original)] == original
            assert summary_count[0] == 2
            results["repeated_manual_compaction"] = {"summary_calls": 2, "summary_allowances": [body["max_output_tokens"] for body in server.requests if summary(body)],
                "fixture_keep_recent_tokens": 1, "raw_prefix_preserved": True,
                "recent_response_preserved": True,
                "prior_summary_in_next_summary": True, "summary_in_actual_provider_input": True,
                "old_image_not_resent": True, "explicit_attachment_reincluded": True}
        finally:
            server.close()
        ...
        for failure_mode in ["incomplete_summary", "compact_cancel"]:
            ...
            try:
                ...
                seeded.close()
            failure = {"type": "response.incomplete", "response": {"status": "incomplete",
                       "incomplete_details": {"reason": "max_output_tokens"}, "usage": {"output_tokens": 10}}}
            server = Server(lambda *_: (200, failure), gate=0 if failure_mode == "compact_cancel" else None)
            try:
                config = configure(destination, composition, server, failure_mode, keep_recent=1)
                if failure_mode == "incomplete_summary":
                    failed = run([host, "session", "compact", failed_history, "--composition", config], caller, check=False)
                    assert failed.returncode != 0 and "ProviderFailure" in failed.stderr
                else:
                    report = json.loads(run([probe, config, project, failed_history, "compact_cancel", server.address], caller).stdout)
                ...
                results[failure_mode] = {"requests": 1, "original_prefix_preserved": True, "no_compaction_committed": True}
            finally:
                server.close()
        ...
        try:
            ...
            server.close()
        server = Server(lambda body, _: (200, complete(answer("threshold-summary" if summary(body) else "after threshold"))))
        try:
            ...
            results["threshold"] = {"configured_window": 50000, "requests": len(server.requests), "summary_in_provider_input": True}
        finally:
            server.close()
        ...
        for mode in ["overflow", "length", "retry_after_tool", "usage_overflow"]:
            ...
            if mode in ["overflow", "length", "usage_overflow"]:
                seed = Server(lambda *_: (200, complete(answer("prior task context " + "h" * 100000))))
                try:
            ...
            def respond(body, index):
                if mode == "overflow" and index == 0:
                    return 400, {"error": {"code": "context_length_exceeded", "message": "controlled overflow"}}
                if mode == "length" and index == 0:
                    return 200, {"type": "response.incomplete", "response": {"status": "incomplete",
                        "incomplete_details": {"reason": "max_output_tokens"}, "usage": {"output_tokens": 10}, "output": effect()}}
                if mode == "retry_after_tool":
                    if index == 0:
                        return 200, complete(effect())
                    assert count_file.read_text() == "x", "historical tool executed again"
                    assert any(item.get("call_id") == "effect" and item["type"] == "function_call_output" for item in body["input"])
                    if index == 1:
                        return 503, {"error": {"code": "server_error"}}
                return 200, complete(answer("recovery-summary" if summary(body) else "recovered"),
            ...
            try:
                ...
                if mode in ["overflow", "length"]:
                    ...
                    assert len(server.requests) == 3 and not any(summary(body) for body in server.requests)
                    ...
                    assert sum(r["kind"] == "compaction" for r in durable) == 1
                results[mode] = {"requests": len(server.requests), "compactions": sum(r["kind"] == "compaction" for r in durable),
                                 "tool_intents": sum(r["kind"] == "tool_intent" for r in durable), "terminal": json.loads(data.stdout.splitlines()[-1])["payload"]}
            finally:
                server.close()
        ...
        for mode in ["one", "all", "cancel_pre", "cancel_post"]:
            ...
            if count_file.exists():
                count_file.unlink()
            server = Server(lambda _, index: (200, complete(effect() if mode == "cancel_post" and index == 0 else answer("queue response"))),
            ...
            try:
                ...
                result = json.loads(run([probe, config, project, queue_history, mode, server.address], caller).stdout)
                ...
                if mode in ["one", "all"]:
                    ...
                    admissions = [r["payload"]["queue_ids"] for r in durable if r["kind"] == "model_request"]
                    expected = [[ids[0]], [ids[1], ids[2]], [ids[3]]] if mode == "one" else [ids[:2], ids[2:]]
                    assert [sorted(group) for group in admissions] == [sorted(group) for group in expected]
                    for line in queue_history.read_text(encoding="utf-8").splitlines():
                        transaction = json.loads(line)["transaction"]
                        for consumed_record in (r for r in transaction if r["kind"] == "queue_consumed"):
                            assert any(r["kind"] == "model_response" and r["payload"]["request_id"] == consumed_record["payload"]["request_id"] for r in transaction)
                    request_ids = {r["payload"]["request_id"] for r in durable if r["kind"] == "model_request"}
                    assert all(r["payload"]["request_id"] in request_ids for r in consumed)
                    final_input = json.dumps(server.requests[-1]["input"])
                    assert all(label in final_input for label in ["steer-one", "steer-two", "follow-one", "follow-two"])
                    first_input = json.dumps(server.requests[0]["input"])
                    assert ("steer-two" in first_input) is (mode == "all")
                elif mode == "cancel_pre":
                    assert len(server.requests) == 1
                    assert not any(r["kind"] in ["queue_consumed", "model_response", "tool_intent"] for r in durable)
                    returned = [r["payload"]["id"] for r in durable if r["kind"] == "queue_returned"]
                    assert returned == ids
                else:
                ...
                results[mode] = {"requests": len(server.requests), "ids": ids, "terminal": result["terminal"]}
            finally:
                server.close()
            if mode.startswith("cancel_"):
                ...
                try:
                    ...
                    result = json.loads(run([probe, config, project, queue_history, "resume", reopened.address], caller).stdout)
                    ...
                    results[mode]["new_process_reopen"] = {"before_explicit_continue_requests": 0,
                        "restored_ids": result["ids"], "requests": 1, "old_tools_not_replayed": True}
                finally:
                    reopened.close()
        assert host.read_bytes() == fixed_host
    evidence = {"platform": platform.platform(), "target": target(),
                "commit": run(["git", "rev-parse", "HEAD"], ROOT).stdout.strip(),
                "source_dirty": bool(run(["git", "status", "--porcelain"], ROOT).stdout.strip()),
                "host_sha256": hashlib.sha256(fixed_host).hexdigest(), "host_unchanged": True,
                "installed_library_sha256": {str(path.relative_to(destination)): hashlib.sha256(path.read_bytes()).hexdigest()
                    for path in sorted((destination / "plugins").rglob("*")) if path.is_file()},

...
import sys
import tempfile
from install import ROOT, install, target

spec = importlib.util.spec_from_file_location("coding_verifier", ROOT / "scripts/verify-coding.py")
helpers = importlib.util.module_from_spec(spec)
spec.loader.exec_module(helpers)
run, write, Server, answer, call, configure, events = (getattr(helpers, name) for name in ["run", "write", "Server", "answer", "call", "configure", "events"])
...
def broken_stdout(host, selected, destination, server, history, caller, verb, arguments):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
        ...
        process = subprocess.Popen([str(host), "session", verb, str(history), *arguments, "--composition", str(config)], cwd=caller, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, encoding="utf-8")
        try:
            ...
            return {"command": verb, "exit_code": process.returncode, "store_open_before_reader_closed": True, "native_store_closed_after_output_failure": True, "error": stderr.strip()}
        finally:
...
def main():
    ...
    run(["cargo", "build", "-p", "eden-agent", "--example", "session_probe", "--locked"], ROOT)
    ...
    with tempfile.TemporaryDirectory(prefix="eden-session-acceptance-") as temp:
        ...
        author["descriptor"]["provides"] = [PROVIDER, CONTEXT, helpers.TOOL, STORE, INTERPRETER, MIGRATOR]
        caller, project, relocated = [scratch / name for name in ["unrelated caller 工作目录", "original project", "relocated project"]]
        for folder in [caller, project, relocated]:
            folder.mkdir()
        def respond(body, index):
            if index == 0:
                return call("origin-write", "write", path="origin-effect.txt", content="recorded cwd effect\n")
            if "MOVED_CONTINUATION" in json.dumps(body["input"]) and not any(item.get("type") == "function_call_output" and item.get("call_id") == "move-write" for item in body["input"]):
                return call("move-write", "write", path="moved-effect.txt", content="relocated cwd effect\n")
        ...
        try:
            ...
            def session(verb, history, *args, selected=config, check=True):
                return command("session", verb, history, *args, "--composition", selected, check=check)
            def submit(history, text, selected=config, cwd=None, check=True):
                args = ["--composition", selected, "--session", history, "--json"]
                if cwd is not None:
                    args.extend(["--cwd", cwd])
            ...
            root = next(record["sequence"] for record in baseline if record["kind"] == "message" and record["payload"].get("role") == "user")
            ...
            assert info["branch"] == "right" and pathlib.Path(info["binding"]["cwd"]).samefile(project)
            tree = lines(session("tree", history))
            assert any(node["branch"] == "right" and node["parent"] == root for node in tree)
            assert (project / "origin-effect.txt").read_text(encoding="utf-8") == "recorded cwd effect\n"
            results["branch_input"] = {"common_ancestor_visible": True, "departed_branch_and_queue_absent": True, "old_nodes_unchanged": len(old), "branch": info["branch"]}
            ...
            results["copies"] = {"preview_has_no_destination": True, "fresh_session_ids": True, "fork_ancestor_only": True, "clone_keeps_tree": True, "pending_not_copied": True, "source_unchanged": source_hash, "preview_losses": preview["losses"]}
            ...
            assert (relocated / "moved-effect.txt").read_text(encoding="utf-8") == "relocated cwd effect\n"
            ...
            results["cwd_binding"] = {"caller_cwd_ignored_on_reopen": True, "conflicting_explicit_cwd_rejected": True, "explicit_copy_binds_new_cwd": True, "real_tool_effect_only_in_relocated_project": True, "missing_cwd_still_readable": True}
            results["stale_preview"] = lines(run([probe, config, project, history, "stale-copy", scratch / "stale.jsonl"], caller))[0]
            ...
            legacy.write_text("".join(json.dumps(record) + "\n" for record in legacy_records), encoding="utf-8")
            ...
            results["upgrade_recovery"] = {"legacy_readable_but_not_writable": True, "legacy_source_sha256": legacy_hash, "damaged_prefix_records": len(lines(prefix)), "damage_source_sha256": damaged_hash, "reopened_copies_completed": True}
            ...
            results["offline_public_history"] = {"all_native_libraries_unavailable": True, "inspect_and_export_succeeded": True, "export_records": len(records(exported)), "existing_destination_rejected": True}

            selected = copy.deepcopy(composition)
            selected["packages"].append(author)
            selected["roles"].update({CONTEXT:"coding-replacements", STORE:"coding-replacements", INTERPRETER:"coding-replacements", MIGRATOR:"coding-replacements"})
            ...
            assert any(record["kind"] == "compaction" and record["payload"].get("author") == "coding-replacements" for record in compacted)
            ...
            session("branch", state_history, "--at", "1", "--branch", "author-carried", "--summarize", selected=independent)
            ...
            carried = [record for record in records(state_history) if record["kind"] == "branch_summary"]
            assert carried and carried[-1]["payload"]["origin_ids"]
            assert "INDEPENDENT_SUMMARY" in json.dumps(server.requests[-1]["input"])
            results["independent_context_and_store"] = {"sdk_only_author": True, "seed": seed, "required_state_observed_in_model_input": True, "summary_observed_after_reopen": True, "raw_records_preserved": len(raw_before), "detached_branch_summary_visible_after_reopen": True}

            requests_before_pipe_failure = len(server.requests)
            results["broken_stdout_cleanup"] = [broken_stdout(host, selected, destination, server, state_history, caller, verb, arguments) for verb, arguments in [("queue", []), ("enqueue", ["BROKEN_PIPE_QUEUE"]), ("metadata", ["--name", "saved before output failure"])]]
            assert len(server.requests) == requests_before_pipe_failure
            pipe_records = records(state_history)
            assert any(record["kind"] == "queue_accepted" and "BROKEN_PIPE_QUEUE" in json.dumps(record["payload"]) for record in pipe_records)
            assert any(record["kind"] == "session_metadata" and record["payload"]["name"] == "saved before output failure" for record in pipe_records)

            unknown = scratch / "unknown-required.jsonl"
            run([probe, independent, project, unknown, "state", "1", "unknown.counter"], caller)
            before = len(server.requests)
            assert submit(unknown, "MUST_NOT_CALL_MODEL", selected=independent, check=False).returncode != 0
            ...
            assert submit(missing_history, "MISSING_INTERPRETER_NO_MODEL", selected=missing_config, check=False).returncode != 0
            assert len(server.requests) == before
            results["required_state_guard"] = {"unknown_namespace_blocks_model": True, "missing_interpreter_blocks_model": True}
            ...
            migrated_state = next(record["payload"] for record in records(migrated) if record["kind"] == "extension_state")
            assert migrated_state["version"] == 1 and migrated_state["value"] == {"count":17,"label":"preserved"}
            ...
            assert session("migrate", old_state, fail_destination, "--apply", selected=fail_config, check=False).returncode != 0
            ...
            assert session("migrate", old_state, false_destination, "--apply", selected=false_config, check=False).returncode != 0
            assert not false_destination.exists() and digest(old_state) == old_hash
            results["independent_migration"] = {"preview_preserved": plan["preserved"], "actual_v0_to_v1_conversion": migrated_state, "interpreter_confirmed_after_reopen": True, "failure_leaves_destination_absent": True, "false_conversion_claim_rejected": True, "source_sha256": old_hash}

            memory = scratch / "memory"
            memory.mkdir()
            events(command("--composition", config, "--cwd", memory, "--no-session", "--json", "MEMORY_SUCCESS"))
            ...
            before = len(server.requests)
            assert command("--composition", bad_memory_config, "--cwd", memory, "--no-session", "--json", "MEMORY_FAILURE", check=False).returncode != 0
            assert len(server.requests) == before and list(memory.iterdir()) == []
            results["memory"] = {"success_and_persistence_failure_create_no_files": True, "failed_commit_prevents_model_call": True}
        finally:
            server.close()
        assert digest(host) == host_hash
    evidence = {"platform":platform.platform(), "target":target(), "commit":run(["git","rev-parse","HEAD"],ROOT).stdout.strip(), "source_dirty":bool(run(["git","status","--porcelain"],ROOT).stdout.strip()), "host_sha256":host_hash, "host_unchanged_after_external_author_build":True, "provider_evidence":"controlled Responses HTTP", "scenarios":results}

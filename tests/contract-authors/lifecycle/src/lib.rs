...
fn io(error: std::io::Error) -> Fault {
    Fault::new("ProviderFailure", "lifecycle-io", error.to_string())
}
struct PanicOnDrop(bool);
impl Drop for PanicOnDrop {
    fn drop(&mut self) {
        if self.0 {
            panic!("deliberate root destructor panic");
        }
    }
}
impl AgentLoop for Lifecycle {
    async fn run(&self, _: RunInput, cx: CallContext) -> Result<String, Fault> {
        ...
        cx.scope.cleanup(async move {
            if !cleanup_observation.load(std::sync::atomic::Ordering::Acquire) {
                return Err(Fault::new(
                    "CleanupFailure",
                    "child-barrier",
                    "cleanup started before child completion",
                ));
            }
        ...
        started_rx
            .await
            .map_err(|_| Fault::new("Unavailable", "child", "child did not start"))?;
        let _guard = PanicOnDrop(self.mode == "drop_panic");
        let mut byte = [0];
        let read = root.read(&mut byte);
        tokio::pin!(read);
        ...
        match self.mode.as_str() {
            "failed_then_cancel" => Err(Fault::new(
                "ProviderFailure",
                "author-root",
                "deliberate root failure",
            )),
...
fn create(config: Value) -> Result<Package, Fault> {
    let address = config
        ...
        .ok_or_else(|| Fault::new("InvalidInput", "lifecycle", "address required"))?
        .to_owned();
    let mode = config
        .get("mode")
        .and_then(Value::as_str)
